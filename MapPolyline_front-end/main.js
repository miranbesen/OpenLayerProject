...
initializeMap = () => {
    ...
    pointInteraction.on("drawend",
        (event) => {
            ...
            $('#geometryType').val(wkt); //de�ere atama yap�yoruz 
            $('#addPopupNumber').val('')
            $('#addPopupName').val('')
            $('#addPopup').show();
...
var addDrawCount = 0; //Ama� olarak add draw yaparken info drawing e bas�lmas�n� �nlemek.
...
document.addEventListener('jspanelbeforeclose', function (event) {
    jspanelCounter = 0;
    InfoInteraction.setActive(false); //Durmadan info butonuna basmak i�in yazd�k. 
}, false);
